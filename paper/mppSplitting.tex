...
c_e(x)=-\log(p(x))
\]
Assuming this function is twice differentiable, it is convex if its second derivative is be nonnegative on the entire domain.
...
\]
In particular we see that the negative log probabilities result in a convex cost function if the following inequality holds:
\[
(p'(x))^2 \geq p(x)p''(x)
\]
